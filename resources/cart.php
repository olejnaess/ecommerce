...
if(isset($_GET['add'])){
    ...
    while($row = fetch_array($query)){
        ...
        if($row['product_qty'] != $_SESSION['product_'. $_GET['add']]){
            ...
            set_message("We only have {$row['product_title']} " . $row['product_qty']." ". " available");
            redirect("../public/checkout.php");
        }
    }
}
...
function show_paypal(){
    ...
    if(isset($_SESSION['item_qty'])&& $_SESSION['item_qty'] > 0){
...
if(isset($_GET['tx'])) {
    ...
    $sendOrder = query("INSERT INTO orders(order_amount, order_transaction, order_status, order_currency) VALUES('{$amount}', '{$currency}', '{$transaction}', '{$status}')");
    $last_id = last_id();
    confirm($sendOrder);
        ...
        if ($value > 0) {
            ...
            if (substr($name, 0, 8) == "product_") {
                ...
                while ($row = fetch_array($query)) {
                    ...
                    $insertReport = query("INSERT INTO reports(product_id, order_id, product_title, product_price, product_qty) VALUES('{$id}','{$last_id}','{$product_title}','{$product_price}','{$value}')");
